...
function exportCSV(alerts) {
  ...
  const rows = alerts.map(a => [
    ...
    a.mitre_tactic || "", a.confidence != null ? a.confidence + "%" : "",
    a.created_at || "",
  ]);
  ...
  a.href = url; a.download = `sentinelops-report-${Date.now()}.csv`; a.click();
...
function exportJSON(alerts) {
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = `sentinelops-report-${Date.now()}.json`; a.click();
  URL.revokeObjectURL(url);
}
...
export default function ReportsView() {
  ...
  const runAI = async () => {
    setAiLoading(true);
    setAiReport("");
    const summary = await freeformQuery(
      `Generate a concise executive-level incident report based on the following alert data: 
       Total alerts: ${total}, Open: ${open}, Critical: ${critical}, Resolved: ${resolved}, False Positives: ${fp}.
       Severity breakdown: ${severityCounts.map(s => s.label + ": " + s.count).join(", ")}.
       Top tactics: ${[...new Set(alerts.map(a => a.mitre_tactic).filter(Boolean))].slice(0,5).join(", ") || "N/A"}.
       Provide: 1) Executive Summary 2) Key Findings 3) Risk Assessment 4) Recommended Actions.`
    );
    setAiReport(summary);
    setAiLoading(false);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          {[
            { label: "Export CSV",  action: () => exportCSV(alerts),  color: "#22d3ee", border: "rgba(34,211,238,0.3)",  bg: "rgba(34,211,238,0.08)"  },
            { label: "Export JSON", action: () => exportJSON(alerts), color: "#a78bfa", border: "rgba(167,139,250,0.3)", bg: "rgba(167,139,250,0.08)" },
            { label: "AI Report",   action: () => { setActiveTab("ai report"); runAI(); }, color: "#818cf8", border: "rgba(129,140,248,0.35)", bg: "rgba(129,140,248,0.12)" },
          ].map(b => (
      ...
      {activeTab === "summary" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              {severityCounts.map(({ label, count }) => {
                const meta = SEV_META[label] || SEV_META.low;
                const pct  = maxCount > 0 ? (count / maxCount) * 100 : 0;
                return (
                  <div key={label}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <Badge label={label.toUpperCase()} {...meta} />
                      </div>
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: "13px", fontWeight: 600, color: meta.color }}>{count}</span>
                    </div>
          ...
          <div style={{
            ...
            {(() => {
              const tactics = alerts.map(a => a.mitre_tactic).filter(Boolean);
        ...
        <div style={{
          ...
          <div style={{ maxHeight: "420px", overflowY: "auto" }}>
            {alerts.length === 0
              ...
              : alerts.map((a, i) => {
                  const meta = SEV_META[a.severity] || SEV_META.low;
                  return (
                    ...
                    >
                      <Badge label={(a.severity || "low").toUpperCase()} {...meta} />
          ...
          <div style={{
            ...
            <button onClick={() => exportCSV(alerts)} style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "10px" }}>
            <div>
              ...
              <div style={{ fontSize: "11px", color: "#5a6480", marginTop: "3px" }}>Powered by SentinelOps AI Engine</div>
            </div>
            <button onClick={runAI} disabled={aiLoading} style={{
              padding: "9px 20px", borderRadius: "8px", cursor: aiLoading ? "not-allowed" : "pointer",
  ...
  <div>
    <div style={{fontWeight:700,fontSize:"18px"}}>Threat Summary</div>
  ...
  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:"16px"}}>
    ...
    <div>
      <div style={{fontSize:"12px",opacity:0.7}}>RISK LEVEL</div>
      <div>{aiReport?.risk_level}</div>
    </div>

    <div>
      <div style={{fontSize:"12px",opacity:0.7}}>CONFIDENCE</div>
      <div>{aiReport?.confidence}</div>
    </div>

    <div>
      <div style={{fontSize:"12px",opacity:0.7}}>ATTACK STAGE</div>
      <div>{aiReport?.attack_stage}</div>
    </div>

  </div>

  <div>
    <div style={{fontWeight:700}}>Indicators of Compromise</div>
    <ul>
      {aiReport?.indicators_of_compromise?.map((ioc,i)=>(
        <li key={i}>{ioc}</li>
      ))}
    </ul>
  </div>
